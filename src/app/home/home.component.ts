...
export class HomeComponent implements OnInit {
  ...
  goToDetail(id){
    ...
    this.router.navigate(['../user-desc/'+id], { relativeTo: this.activatedRoute }); 
 
  }

  getUsers(){
    this.users =[];
      this.httpClient.get('https://final-web-e2e70.firebaseio.com/users.json').subscribe(result=>{
  ...
  deleteContact(id){
    this.httpClient.delete(`https://final-web-e2e70.firebaseio.com/users/${id}.json`).subscribe(result=>{
      console.log(result);
      this.ngOnInit();
  }, error=>{
